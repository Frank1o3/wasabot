            ...
            try:
                ...
                if response.status_code == 200:
                    logger.info(
                        ...
                        extra={
                            "meta": {"correlation_id": correlation_id, "message_id": message_id}
                        },
                    )
                    return
                ...
                logger.warning(
                    f"typing_indicator_timeout | attempt={attempt + 1} | error={e!s}",
                    extra={"meta": {"correlation_id": correlation_id}},
                )
                if attempt < 1:
                    await asyncio.sleep(1)
                ...
                logger.error(
                    f"typing_indicator_request_error | error={e!s}",
                    extra={"meta": {"correlation_id": correlation_id}},
                )
                return
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        for attempt in range(2):  # Max 2 attempts
            try:
                ...
                logger.warning(
                    f"mark_read_timeout | attempt={attempt + 1} | error={e!s}",
                    extra={"meta": {"correlation_id": correlation_id}},
                )
                if attempt < 1:
                    await asyncio.sleep(1)
                ...
                logger.error(
                    f"mark_read_request_error | error={e!s}",
                    extra={"meta": {"correlation_id": correlation_id}},
                )
                return
